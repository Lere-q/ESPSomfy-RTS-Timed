...
void SchedulerClass::executeSchedule(Schedule& schedule) {
    ...
    // 2. String → somfy_commands (FIX: Nur einmal konvertieren!)
    String cmdStr = String(schedule.command);
    somfy_commands cmd = translateSomfyCommand(cmdStr);

    // 3. Logging (FIX: cmdStr statt erneut translateSomfyCommand)
    Serial.printf(
    ...
    // 4. ✅ SEND-AUFRUF
    Serial.printf("Scheduler: Calling shade->sendCommand(cmd=%d, repeat=1)\n", (int)cmd);
    shade->sendCommand(cmd, 1);
    Serial.println("Scheduler: sendCommand() abgeschlossen");
}
