...
// ========== PROFESSIONELLE ZEITSTEUERUNGS-KLASSE (FINAL) ==========
class TimeScheduler {
    initialized = false;
    schedules = [];
    shadesList = [];
    ...
    init() {
        if (this.initialized) return;
        this.initialized = true;
        console.log('✓ TimeScheduler initialisiert (Server-basiert)');
    }

    toggleScheduler() {
        let div = document.getElementById('divTimeScheduler');
        let isVisible = div.style.display !== 'none';
        div.style.display = isVisible ? 'none' : '';
        
        if (!isVisible) {
            this.loadShadeOptions();
            this.loadSchedules();
        }
    }

    loadShadeOptions() {
        let shadeControls = document.querySelectorAll('.somfyShadeCtl');
        this.shadesList = [];
        
        shadeControls.forEach(ctrl => {
            let shadeId = ctrl.getAttribute('data-shadeid');
            let nameEl = ctrl.querySelector('.shadectl-name');
            let name = nameEl ? nameEl.textContent.trim() : `Gerät ${shadeId}`;
            
            this.shadesList.push({
                shadeId: parseInt(shadeId),
                name: name
            });
        });
        
        let sel = document.getElementById('selScheduleShade');
        if (sel) {
            sel.innerHTML = '<option value="">Gerät auswählen...</option>';
            this.shadesList.forEach(shade => {
                let opt = document.createElement('option');
                opt.value = shade.shadeId;
                opt.text = shade.name;
                sel.appendChild(opt);
            });
        }
    ...
    saveSchedule() {
        let shadeId = document.getElementById('selScheduleShade').value;
        let time = document.getElementById('fldScheduleTime').value;
        let command = document.getElementById('selScheduleCommand').value;
        
        if (!shadeId) {
            ui.errorMessage('Bitte wähle ein Gerät aus.');
            return;
        }
        
        if (!time) {
            ui.errorMessage('Bitte wähle eine Uhrzeit aus.');
            return;
        }
        
        // FIX: Sammle Tage als Bitmask statt Array
        let daysBitmask = 0;
        document.querySelectorAll('.schedule-day:checked').forEach(cb => {
            let dayValue = parseInt(cb.value);
            daysBitmask |= (1 << dayValue);
        });
        
        if (daysBitmask === 0) {
            ui.errorMessage('Bitte wähle mindestens einen Wochentag aus.');
            return;
        }
        
        let shade = this.shadesList.find(s => s.shadeId == shadeId);
        
        // FIX: Sende Bitmask statt Array
        let schedule = {
            shadeId: parseInt(shadeId),
            shadeName: shade ? shade.name : `Gerät ${shadeId}`,
            time: time,
            command: command,
            days: daysBitmask,
            enabled: true
        ...
        console.log('Speichere Zeitplan:', schedule);
        console.log('Tage Bitmask:', daysBitmask, '(binär:', daysBitmask.toString(2).padStart(7, '0'), ')');
        
        // FIX: Ladeanimation-Bug - Callback richtig verwenden
        putJSONSync('/saveSchedule', schedule, (err, response) => {
            if (err) {
                ui.serviceError(err);
            } else {
                // FIX: Callback MUSS aufgerufen werden um Ladeanimation zu stoppen
                ui.infoMessage('✓ Zeitplan erfolgreich gespeichert!<br><small>Läuft automatisch auf dem ESP32</small>', () => {
                    this.loadSchedules();
                    this.resetForm();
                });
            }
    ...
    resetForm() {
        document.getElementById('fldScheduleTime').value = '';
        document.getElementById('selScheduleShade').selectedIndex = 0;
        
        // FIX: Setze Wochentags-Buttons zurück
        document.querySelectorAll('.weekday-btn').forEach(btn => {
            let checkbox = btn.querySelector('.schedule-day');
    ...
    loadSchedules() {
        getJSON('/getSchedules', (err, schedules) => {
            if (err) {
                console.log('Fehler beim Laden:', err);
                this.displaySchedules([]);
            } else {
                this.schedules = schedules || [];
                console.log('Geladene Zeitpläne:', this.schedules);
    ...
    displaySchedules(schedules) {
        let div = document.getElementById('divScheduleList');
        let statsDiv = document.getElementById('schedulerStats');
        
        if (!div) return;
        
        // Statistiken berechnen
        let activeCount = schedules.filter(s => s.enabled).length;
        let totalCount = schedules.length;
        
        if (statsDiv) {
            statsDiv.innerHTML = `
                <div class="stat-item">📊 Gesamt: ${totalCount}</div>
                <div class="stat-item">✓ Aktiv: ${activeCount}</div>
                <div class="stat-item">⏸️ Inaktiv: ${totalCount - activeCount}</div>
            `;
        }
        ...
        if (schedules.length === 0) {
            div.innerHTML = `
                <div class="empty-calendar">
                    ...
                    <div class="empty-text">Keine Zeitpläne vorhanden</div>
                    <div class="empty-hint">Erstelle deinen ersten Zeitplan im Formular links</div>
                </div>`;
            return;
        }
        
        // FIX: Extrahiere Stunde und Minute aus Zeit-String
        let sortedSchedules = [...schedules].sort((a, b) => {
            let [hourA, minuteA] = a.time.split(':').map(Number);
            let [hourB, minuteB] = b.time.split(':').map(Number);
        ...
        sortedSchedules.forEach(schedule => {
            let statusClass = schedule.enabled ? '' : ' disabled';
            let daysText = this.formatDaysDetailed(schedule.days);
            let cmdText = this.getCommandText(schedule.command);
            let nextRun = this.getNextRunDetailed(schedule);
            
            console.log(`Zeitplan: ${schedule.shadeName}, Tage-Bitmask: ${schedule.days} (binär: ${schedule.days.toString(2).padStart(7, '0')})`);
                ...
                <div class="schedule-card${statusClass}">
                    <div class="schedule-header">
                        <div class="schedule-title">
                            ...
                            <div class="schedule-time">${schedule.time} Uhr</div>
                        </div>
                        <div class="schedule-actions">
                            <button class="action-btn btn-toggle ${schedule.enabled ? '' : 'disabled'}" 
                                    onclick="timeScheduler.toggleSchedule(${schedule.id}, ${!schedule.enabled});" 
                                    title="${schedule.enabled ? 'Deaktivieren' : 'Aktivieren'}">
                                ${schedule.enabled ? '✓' : '✕'}
                            </button>
                            <button class="action-btn btn-delete" 
                                    onclick="timeScheduler.deleteSchedule(${schedule.id});" 
                                    title="Löschen">
                                🗑️
                            </button>
                        </div>
                    ...
                    <div class="schedule-details">
                        <div class="detail-item">
                            <span class="command-badge">${cmdText}</span>
                        </div>
                        <div class="detail-item">
                            <div class="days-list">${daysText}</div>
                        </div>
                    </div>
                    ...
                    <div class="next-run-banner">
                        ...
                        <div style="flex:1;">
                            <div class="next-run-text">Nächste Ausführung</div>
                            <div class="next-run-time">${nextRun}</div>
                        </div>
                    </div>
                    ` : ''}
                </div>`;
    ...
    formatDaysDetailed(daysBitmask) {
        ...
        console.log(`Bitmask ${daysBitmask} → Tage:`, days);
        
        if (days.length === 7) {
            return '<div class="day-badge daily-badge">Täglich</div>';
        }
        
        if (days.length === 5 && days.includes(1) && days.includes(2) && days.includes(3) && days.includes(4) && days.includes(5)) {
            return '<div class="day-badge weekday-badge">Mo-Fr</div>';
        }
        
        if (days.length === 2 && days.includes(0) && days.includes(6)) {
            return '<div class="day-badge weekend-badge">Wochenende</div>';
        }
        ...
        return sortedDays.map(d => `<div class="day-badge">${dayNames[d]}</div>`).join('');
    }

    getNextRunDetailed(schedule) {
        if (!schedule.enabled) return null;
        
        let now = new Date();
        let [hours, minutes] = schedule.time.split(':').map(Number);
        ...
        if (schedule.days & (1 << currentDay)) {
            if (targetMinutes > currentMinutes) {
                ...
                if (diff < 60) {
                    return `in ${diff} Minute${diff !== 1 ? 'n' : ''}`;
                } else if (diff < 1440) {
                    let hours = Math.floor(diff / 60);
                    let mins = diff % 60;
                    return `in ${hours} Std${mins > 0 ? ` ${mins} Min` : ''}`;
                }
                return 'Heute';
            }
        }
        ...
        for (let i = 1; i <= 7; i++) {
            let checkDay = (currentDay + i) % 7;
            if (schedule.days & (1 << checkDay)) {
                if (i === 1) return `Morgen um ${schedule.time} Uhr`;
                return `${dayNames[checkDay]} um ${schedule.time} Uhr`;
            }
        }
    ...
    formatDays(daysBitmask) {
        ...
        if (days.length === 5 && days.includes(1) && days.includes(2) && days.includes(3) && days.includes(4) && days.includes(5)) {
            return 'Mo-Fr (Werktags)';
        }
        if (days.length === 2 && days.includes(0) && days.includes(6)) {
            return 'Sa-So (Wochenende)';
        }
    ...
    toggleSchedule(id, enabled) {
        putJSONSync('/toggleSchedule', { id: id, enabled: enabled }, (err, response) => {
            if (err) {
                ui.serviceError(err);
            } else {
                this.loadSchedules();
    ...
    deleteSchedule(id) {
        ui.promptMessage('Möchtest du diesen Zeitplan wirklich löschen?', () => {
            putJSONSync('/deleteSchedule', { id: id }, (err, response) => {
                if (err) {
                    ui.serviceError(err);
                } else {
                    this.loadSchedules();
    ...
    exportSchedules() {
        if (this.schedules.length === 0) {
            ui.errorMessage('Keine Zeitpläne zum Exportieren vorhanden.');
            return;
        }
        ...
        a.href = url;
        a.download = `zeitplaene_backup_${new Date().toISOString().split('T')[0]}.json`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);

        ui.infoMessage(`✓ ${this.schedules.length} Zeitpläne exportiert!`);
    }
            ...
            reader.onload = (event) => {
                try {
                    ...
                    if (!data.schedules || !Array.isArray(data.schedules)) {
                        ui.errorMessage('Ungültiges Backup-Format!');
                        return;
                    }

                    ui.promptMessage(
                        `Backup enthält ${data.schedules.length} Zeitpläne.<br>` +
                        `Export-Datum: ${new Date(data.exportDate).toLocaleString('de-DE')}<br><br>` +
                        `Möchtest du diese importieren?<br>` +
                        `<small>(Bestehende Zeitpläne werden nicht überschrieben)</small>`,
                        () => {
                            this.performImport(data.schedules);
                        }
                    );
                } catch (err) {
                    console.error('Import-Fehler:', err);
                    ui.errorMessage('Fehler beim Lesen der Backup-Datei: ' + err.message);
                }
            };
    ...
    performImport(importedSchedules) {
        let successCount = 0;
        let errorCount = 0;
        let totalCount = importedSchedules.length;

        let importNext = (index) => {
            if (index >= totalCount) {
                ui.infoMessage(
                    `Import abgeschlossen!<br>` +
                    `✓ Erfolgreich: ${successCount}<br>` +
                    `${errorCount > 0 ? `✗ Fehler: ${errorCount}` : ''}`,
                    () => {
                        this.loadSchedules();
                    }
                );
            ...
            let schedule = importedSchedules[index];
            
            // Entferne ID (wird vom Server neu vergeben)
            delete schedule.id;
            delete schedule.lastRun;

            putJSONSync('/saveSchedule', schedule, (err, response) => {
                if (err) {
                    console.error('Fehler beim Importieren:', schedule, err);
                    errorCount++;
                } else {
                    successCount++;
                }
    ...
    deleteAllSchedules() {
        if (this.schedules.length === 0) {
            ui.errorMessage('Keine Zeitpläne zum Löschen vorhanden.');
            return;
        }

        ui.promptMessage(
            `⚠️ ACHTUNG: Alle ${this.schedules.length} Zeitpläne löschen?<br>` +
            `<small>Diese Aktion kann nicht rückgängig gemacht werden!</small>`,
            () => {
                let deleteNext = (index) => {
                    if (index >= this.schedules.length) {
                        ui.infoMessage('✓ Alle Zeitpläne gelöscht!', () => {
                            this.loadSchedules();
                        });
                        return;
                    }
                    ...
                    putJSONSync('/deleteSchedule', { id: schedule.id }, (err) => {
                        if (err) {
                            console.error('Fehler beim Löschen:', schedule.id);
                        }
                        deleteNext(index + 1);
                    });
                };
...
(function() {
    function addButton() {
        ...
        if (h1 && !document.getElementById('btnTimeScheduler')) {
            let btn = document.createElement('span');
            btn.id = 'btnTimeScheduler';
            btn.className = 'button-outline';
            btn.style.cssText = 'float:right;font-size:1.25rem;display:inline-block;vertical-align:middle;width:38px;height:38px;position:relative;padding-top:4px;margin-right:10px;cursor:pointer;';
            btn.innerHTML = '<span style="vertical-align:middle;">⏰</span>';
            btn.onclick = function() { timeScheduler.toggleScheduler(); };
            
            let configBtn = h1.querySelector('.button-outline');
            if (configBtn) h1.insertBefore(btn, configBtn);
            else h1.appendChild(btn);
            
            console.log('✓ Zeitsteuerungs-Button hinzugefügt');
        } else if (!h1) {
