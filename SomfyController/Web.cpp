...
void Web::begin() {
  ...
  // ========== SCHEDULER API ENDPUNKTE (OPTIMIERT) ==========
  // GET /getSchedules
  server.on("/getSchedules", []() {
    Serial.println("API: /getSchedules");
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    
    String json = scheduler.toJSON();
    server.send(200, _encoding_json, json);
    Serial.printf("  → %d Zeitpläne gesendet\n", scheduler.getAllSchedules().size());
  });

  // PUT /saveSchedule
  server.on("/saveSchedule", []() {
    Serial.println("API: /saveSchedule");
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    ...
    if (method == HTTP_PUT || method == HTTP_POST) {
      if (server.hasArg("plain")) {
        ...
        // Shade ID
        if (!obj.containsKey("shadeId")) {
          server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"shadeId required\"}"));
          return;
        }
        schedule.shadeId = obj["shadeId"];
        
        // FIX: Time Parsing
        if (!obj.containsKey("time")) {
          server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"time required\"}"));
          return;
        }
        ...
        if (colonPos <= 0) {
          Serial.printf("  → Invalid time: %s\n", timeStr.c_str());
          server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Invalid time format\"}"));
          return;
        }
        
        schedule.hour = timeStr.substring(0, colonPos).toInt();
        schedule.minute = timeStr.substring(colonPos + 1).toInt();
        
        if (schedule.hour > 23 || schedule.minute > 59) {
          server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Invalid time values\"}"));
          return;
        }
        ...
        if (obj.containsKey("days")) {
          if (obj["days"].is<int>()) {
            schedule.days = obj["days"].as<uint8_t>();
          } else if (obj["days"].is<JsonArray>()) {
            JsonArray daysArray = obj["days"];
            for (JsonVariant day : daysArray) {
              uint8_t d = day.as<uint8_t>();
              if (d <= 6) schedule.days |= (1 << d);
            }
          }
        }
        
        if (schedule.days == 0) {
          server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"At least one day required\"}"));
          return;
        }
        ...
        if (obj.containsKey("command")) {
          ...
          strlcpy(schedule.command, cmd.c_str(), sizeof(schedule.command));
        } else {
          strlcpy(schedule.command, "up", sizeof(schedule.command));
        }
        
        schedule.enabled = obj.containsKey("enabled") ? obj["enabled"].as<bool>() : true;
        
        // FIX: Shade Name mit Buffer-Schutz
        if (obj.containsKey("shadeName")) {
          String name = obj["shadeName"].as<String>();
          strlcpy(schedule.shadeName, name.c_str(), sizeof(schedule.shadeName));
        } else {
          SomfyShade* shade = somfy.getShadeById(schedule.shadeId);
          if (shade) {
            strlcpy(schedule.shadeName, shade->name, sizeof(schedule.shadeName));
          } else {
            snprintf(schedule.shadeName, sizeof(schedule.shadeName), "Shade %d", schedule.shadeId);
          }
        }
        ...
        Serial.printf("  → Tage: 0x%02X\n", schedule.days);
        Serial.printf("  → Cmd: %s\n", schedule.command);
        
        if (scheduler.addSchedule(schedule)) {
          String json = scheduler.scheduleToJSON(schedule);
          server.send(200, _encoding_json, json);
          Serial.printf("  → Erfolg! ID: %u\n", schedule.id);
        } else {
          server.send(500, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Could not save\"}"));
        }
      } else {
        server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"No data\"}"));
      }
    } else {
      server.send(405, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Invalid method\"}"));
    }
  });

  // PUT /updateSchedule
  server.on("/updateSchedule", []() {
    Serial.println("API: /updateSchedule");
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    ...
    if (method == HTTP_PUT || method == HTTP_POST) {
      if (server.hasArg("plain")) {
        ...
        if (!obj.containsKey("id")) {
          server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"id required\"}"));
          return;
        }
        ...
        if (scheduler.updateSchedule(id, schedule)) {
          String json = scheduler.scheduleToJSON(schedule);
          server.send(200, _encoding_json, json);
        } else {
          server.send(404, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Not found\"}"));
        }
      } else {
        server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"No data\"}"));
      }
  ...
  server.on("/deleteSchedule", []() {
    Serial.println("API: /deleteSchedule");
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    ...
    if (method == HTTP_PUT || method == HTTP_POST || method == HTTP_DELETE) {
      ...
      if (scheduler.deleteSchedule(id)) {
        server.send(200, _encoding_json, F("{\"status\":\"SUCCESS\"}"));
        Serial.printf("  → Deleted ID %u\n", id);
      } else {
        server.send(404, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Not found\"}"));
      }
    } else {
  ...
  // PUT /toggleSchedule
  server.on("/toggleSchedule", []() {
    Serial.println("API: /toggleSchedule");
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    ...
    if (method == HTTP_PUT || method == HTTP_POST) {
      if (server.hasArg("plain")) {
        ...
        uint32_t id = obj["id"];
        bool enabled = obj.containsKey("enabled") ? obj["enabled"].as<bool>() : true;
