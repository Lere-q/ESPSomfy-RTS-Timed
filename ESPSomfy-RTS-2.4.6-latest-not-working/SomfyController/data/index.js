...
// ========== PROFESSIONELLE ZEITSTEUERUNGS-KLASSE (OPTIMIERT) ==========
class TimeScheduler {
    initialized = false;
    schedules = [];
    shadesList = [];
    ...
    saveSchedule() {
        ...
        let schedule = {
            ...
            time: time,
            command: command,
            days: daysBitmask,  // Als Bitmask senden
            enabled: true
        };
    ...
    formatDaysDetailed(daysBitmask) {
        ...
        // FIX: Korrekte Bitmask-Extraktion
        for (let i = 0; i < 7; i++) {
            if (daysBitmask & (1 << i)) {
                days.push(i);
            }
        ...
        // Wochenende (Sa-So = 6,0)
        if (days.length === 2 && days.includes(0) && days.includes(6)) {
            return '<div class="day-badge weekend-badge">Wochenende</div>';
        }
        
        // FIX: Sortiere Tage korrekt (Mo=1 bis So=0, aber So am Ende anzeigen)
        let sortedDays = days.sort((a, b) => {
            // Montag=1 ist der erste Tag, Sonntag=0 ist der letzte
            let orderA = a === 0 ? 7 : a;
            let orderB = b === 0 ? 7 : b;
    ...
    getNextRunDetailed(schedule) {
        ...
        // FIX: Prüfe ob heute noch möglich (mit Bitmask)
