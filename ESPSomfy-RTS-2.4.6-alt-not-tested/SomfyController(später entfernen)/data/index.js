...
// ========== PROFESSIONELLE ZEITSTEUERUNGS-KLASSE ==========
class TimeScheduler {
    initialized = false;
    schedules = [];
    shadesList = [];
...
displaySchedules(schedules) {
    ...
    let sortedSchedules = [...schedules].sort((a, b) => {
        let timeA = a.hour * 60 + a.minute;
        let timeB = b.hour * 60 + b.minute;
        return timeA - timeB;
    });
...
formatDaysDetailed(daysBitmask) {
    ...
    if (days.length === 7) {
        return '<div class="day-badge" style="background:rgba(76,175,80,0.3);border-color:#4caf50;color:#4caf50;">Täglich</div>';
    }
    
    if (days.length === 5 && !days.includes(0) && !days.includes(6)) {
        return '<div class="day-badge" style="background:rgba(33,150,243,0.3);border-color:#2196f3;color:#2196f3;">Mo-Fr</div>';
    }
    
    if (days.length === 2 && days.includes(0) && days.includes(6)) {
        return '<div class="day-badge" style="background:rgba(255,152,0,0.3);border-color:#ff9800;color:#ff9800;">Wochenende</div>';
    }
    
    return days.sort((a,b) => {
        // Sortiere Mo-So (1,2,3,4,5,6,0)
        let orderA = a === 0 ? 7 : a;
        let orderB = b === 0 ? 7 : b;
        return orderA - orderB;
    }).map(d => `<div class="day-badge">${dayNames[d]}</div>`).join('');
}
...
getNextRunDetailed(schedule) {
    ...
    formatDays(daysBitmask) {
        ...
        if (days.length === 5 && !days.includes(0) && !days.includes(6)) return 'Mo-Fr (Werktags)';
        if (days.length === 2 && days.includes(0) && days.includes(6)) return 'Sa-So (Wochenende)';
