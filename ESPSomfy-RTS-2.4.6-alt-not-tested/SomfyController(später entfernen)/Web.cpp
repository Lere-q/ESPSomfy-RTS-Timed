...
void Web::begin() {
  ...
  server.on("/endFrequencyScan", []() {
    ...
    */
    ...
    // ========== SCHEDULER API ENDPUNKTE - HIER EINFÜGEN ==========
  
  // GET /getSchedules - Alle Zeitpläne abrufen
  ...
  // PUT /saveSchedule - Neuen Zeitplan erstellen
  server.on("/saveSchedule", []() {
    ...
    if (method == HTTP_PUT || method == HTTP_POST) {
      if (server.hasArg("plain")) {
        ...
        if (colonPos < 0) {
          server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Invalid time format. Use HH:MM\"}"));
          return;
        }
        ...
        if (schedule.hour > 23 || schedule.minute > 59) {
          server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Invalid time\"}"));
          return;
        }
        
        // Parse Wochentage (Array zu Bitmaske)
        schedule.days = 0;
        ...
        if (scheduler.addSchedule(schedule)) {
          ...
          server.send(500, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Could not save schedule\"}"));
        }
      } else {
        server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"No data provided\"}"));
      }
    } else {
      server.send(405, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Invalid HTTP Method\"}"));
    }
  });

  // PUT /updateSchedule - Zeitplan aktualisieren
    ...
    if (method == HTTP_PUT || method == HTTP_POST) {
      if (server.hasArg("plain")) {
        ...
        if (scheduler.updateSchedule(id, schedule)) {
          ...
          server.send(404, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"Schedule not found\"}"));
        }
      } else {
        server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"No data provided\"}"));
      }
  ...
  // PUT /deleteSchedule - Zeitplan löschen
  ...
  // PUT /toggleSchedule - Zeitplan aktivieren/deaktivieren
  server.on("/toggleSchedule", []() {
    ...
    if (method == HTTP_PUT || method == HTTP_POST) {
      if (server.hasArg("plain")) {
        ...
        server.send(200, _encoding_json, F("{\"status\":\"SUCCESS\",\"desc\":\"Schedule updated\"}"));
      } else {
        server.send(400, _encoding_json, F("{\"status\":\"ERROR\",\"desc\":\"No data provided\"}"));
      }
    } else {
